...
export function AccountSessionWorkspace({ user }: { user: UserAccount }) {
  ...
  return (
    <Card className="rounded-[32px] border border-[#7cff00]/16 bg-[linear-gradient(135deg,rgba(124,255,0,0.10),rgba(8,8,8,0.98))] p-6" data-testid="account-session-workspace">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          <p className="surface-label text-[#d7ffab]">Account session</p>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-[24px] border border-white/8 bg-black/22 p-4">
          <div className="inline-flex items-center gap-2 text-sm text-white/78">
            <RotateCcw className="h-4 w-4 text-[#baff69]" />
            Resume target
          </div>
