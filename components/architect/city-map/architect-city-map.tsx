    ...
    <main className="min-h-screen overflow-x-hidden bg-[#060708] text-[#F5F1E8] [-webkit-text-size-adjust:100%] [text-size-adjust:100%]" data-testid="architect-city-map">
      ...
      <div className="relative z-10">
        ...
        <section className="mx-4 mb-3 grid gap-3 sm:mx-7 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="rounded-[18px] border border-white/[0.08] bg-[#08090A]/72 p-4"><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#C9A87C]">Alert history</p><div className="mt-4 flex min-h-20 gap-2 overflow-x-auto border-t border-white/[0.08] pt-4">{manifest.incidents.length ? manifest.incidents.map((incident) => <article key={incident.id} className={cn("min-w-[160px] rounded-[12px] border p-3 text-left", incident.severity === "P0" ? "border-[#FF9B9B]/35" : "border-[#D9B461]/25")}><span className="font-mono text-[8px] text-white/32">{incident.severity} · {formatTime(incident.occurredAt)}</span><span className="mt-2 block text-[11px] leading-4 text-white/60">{incident.message}</span></article>) : <span className="text-sm text-white/34">No open incidents returned by Mission Control.</span>}</div></div>
