...
function severityClass(severity: ProfileStudioSeverity = "neutral") {
  if (severity === "good") {
    return "border-[#c4f24e]/20 bg-[rgba(196, 242, 78,0.08)]";
  }
  if (severity === "warning") {
...
export function ProfileStudioShell({
  ...
  async function handleSetFeaturedMedia(itemId: string) {
    if (!onSetFeaturedMedia) {
      return;
    }

    setFeaturedMediaFeedback(null);
    setPendingFeaturedMediaId(itemId);
    try {
      await onSetFeaturedMedia(itemId);
      setFeaturedMediaFeedback("Featured image saved.");
    } catch {
      setFeaturedMediaFeedback("Unable to set featured image.");
    } finally {
      setPendingFeaturedMediaId(null);
    }
  }
  ...
  return (
    <div className="space-y-6" data-testid={`profile-studio-${model.role}`}>
      <GlassCard className="flex flex-wrap items-start justify-between gap-4 rounded-[22px] p-5 sm:p-6">
        ...
        <Link
          href={backHref}
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-4 text-sm font-extrabold text-white/76 transition hover:border-[#c4f24e]/30 hover:text-[#c4f24e]"
        >
      ...
      {model.page.statusText ? (
        <GlassCard className="flex items-center gap-3 rounded-[18px] p-4">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#c4f24e]/25 bg-[#c4f24e]/10 text-[#c4f24e]">
      ...
      <GlassCard active className="relative overflow-hidden rounded-[28px] p-0">
        <div
          className="h-44 border-b border-white/8 bg-[radial-gradient(circle_at_top_right,rgba(196, 242, 78,0.18),transparent_34%),linear-gradient(135deg,rgba(196, 242, 78,0.10),rgba(255,255,255,0.04)_42%,rgba(0,0,0,0.34))]"
        ...
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[auto_minmax(0,1fr)]">
          <div className="relative -mt-20 h-[148px] w-[148px] shrink-0 overflow-hidden rounded-[28px] border-[3px] border-white/15 bg-black text-4xl font-black text-[#c4f24e] shadow-[0_0_0_2px_rgba(196, 242, 78,0.10),0_20px_60px_rgba(0,0,0,0.50)] sm:h-[178px] sm:w-[178px] sm:rounded-[36px] sm:text-5xl">
          ...
          <div className="min-w-0">
            ...
            {model.hero.username ? (
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.035] text-[#c4f24e] transition hover:border-[#c4f24e]/30 hover:bg-[#c4f24e]/10"
                  onClick={openUsernameModal}
                >
                  <Pencil className="h-4 w-4" />
                </button>
                {usernameFeedback ? <span className="text-xs font-bold text-[#c4f24e]">{usernameFeedback}</span> : null}
              </div>
            ) : null}
            <div className="mt-4 flex max-w-3xl flex-wrap items-center gap-2">
              ...
              {bioFeedback ? <span className="text-xs font-bold text-[#c4f24e]">{bioFeedback}</span> : null}
            </div>
            {contextValue ? (
              <div className="mt-2 flex flex-wrap items-center gap-2 text-base font-semibold text-white/50">
                <span>{contextValue}</span>
                {contextFeedback ? <span className="text-xs font-bold text-[#c4f24e]">{contextFeedback}</span> : null}
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button type="button" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[8px] bg-[#c4f24e] px-4 text-sm font-black text-[#050505] transition hover:bg-[#e4f9b8]" onClick={onPreview}>
                <Eye className="h-4 w-4" aria-hidden="true" />
                {model.actions.publicPreviewLabel}
              </button>
              <button type="button" aria-label={model.actions.shareLabel} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[8px] border border-white/10 bg-white/[0.035] px-4 text-sm font-extrabold text-white/74 transition hover:border-[#c4f24e]/30 hover:text-white" onClick={onShare}>
                <Share2 className="h-4 w-4 text-[#c4f24e]" aria-hidden="true" />
                Share
              </button>
            </div>
          </div>
        ...
        <div className="grid sm:grid-cols-3">
          {model.stats.map((stat, index) => (
            <div key={stat.label} className={cn("min-h-[128px] p-5", index > 0 && "border-t border-white/10 sm:border-l sm:border-t-0")}>
              ...
              {stat.helper ? <p className="mt-1 text-[15px] font-bold text-[#c4f24e]">{stat.helper}</p> : null}
            </div>
          ))}
        </div>
      </GlassCard>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {model.trustCards.slice(0, 3).map((card) => (
          <GlassCard key={card.title} className={cn("rounded-[20px] p-5", severityClass(card.status))}>
            ...
            <p className="mt-1 text-[15px] font-bold text-[#c4f24e]">{card.helper}</p>
          </GlassCard>
        ))}
      </div>
      ...
      <GlassCard className="flex min-h-[106px] items-center justify-between gap-4 rounded-[18px] p-[22px]">
        <div className="flex min-w-0 items-center gap-4">
          <span className="flex h-[58px] w-[58px] shrink-0 items-center justify-center rounded-full border border-[#c4f24e]/25 bg-[rgba(196, 242, 78,0.08)] text-[#c4f24e]">
      ...
      <section className="overflow-hidden" aria-label="Profile highlights">
        <div className="hide-scrollbar flex gap-5 overflow-x-auto pb-2">
          <button type="button" className="flex w-[112px] shrink-0 flex-col items-center" onClick={() => setIsFolderModalOpen(true)}>
            <span className="flex h-[112px] w-[112px] items-center justify-center rounded-full border-[3px] border-white/15 bg-white/[0.018] text-[54px] font-light leading-none text-[#c4f24e]">
              +
            </span>
          ...
          {folders.map((folder) => (
            <button key={folder.id} type="button" className="flex w-[112px] shrink-0 flex-col items-center" onClick={() => openFolder(folder)}>
              <span className="flex h-[112px] w-[112px] items-center justify-center rounded-full border-[3px] border-white/15 bg-white/[0.018] text-[38px] font-black leading-none text-[#c4f24e]">
      ...
      <section ref={workSectionRef} className="space-y-4 pb-[max(2.5rem,env(safe-area-inset-bottom))]">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            {featuredMediaFeedback ? <p className="mt-1 text-sm font-bold text-[#c4f24e]">{featuredMediaFeedback}</p> : null}
            {cultureShareFeedback ? <p className="mt-1 text-sm font-bold text-[#c4f24e]">{cultureShareFeedback}</p> : null}
          </div>
          <button type="button" aria-label={mediaButtonLabel} className="inline-flex min-h-11 items-center gap-2 rounded-[8px] border border-[#c4f24e]/25 bg-[#c4f24e]/10 px-4 text-sm font-black text-[#c4f24e] transition hover:bg-[#c4f24e]/16" onClick={handleAddMediaAction}>
          ...
          {model.work.items.length ? model.work.items.slice(0, 9).map((item) => (
            <div key={item.id} className="group relative aspect-square overflow-hidden rounded-[12px] border border-white/10 bg-black/25">
              ...
              <button
                ...
                className="absolute left-2 top-2 inline-flex h-9 w-9 items-center justify-center rounded-full border border-black/40 bg-black/72 text-white shadow-lg transition hover:border-[#c4f24e]/40 hover:text-[#c4f24e]"
              ...
              {onSetFeaturedMedia ? (
                <button
                  type="button"
                  aria-label={item.featured ? "Featured banner" : "Set as featured banner"}
                  disabled={Boolean(isSettingFeaturedMedia) || pendingFeaturedMediaId === item.id}
                  className={cn(
                    "absolute bottom-2 left-2 inline-flex h-9 w-9 items-center justify-center rounded-full border border-black/40 bg-black/72 text-white shadow-lg transition hover:border-[#c4f24e]/40 hover:text-[#c4f24e] disabled:cursor-wait disabled:opacity-70",
                    item.featured && "border-[#c4f24e]/50 bg-[#c4f24e] text-black hover:text-black"
                  )}
              ...
              {onShareMediaToCulture ? (
                <button
                  ...
                  className="absolute bottom-2 right-2 inline-flex h-9 w-9 items-center justify-center rounded-full border border-black/40 bg-black/72 text-white shadow-lg transition hover:border-[#c4f24e]/40 hover:text-[#c4f24e] disabled:cursor-wait disabled:opacity-70"
      ...
      {isFolderModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/72 px-4 py-6 backdrop-blur-md sm:items-center">
          <div role="dialog" aria-modal="true" aria-label="Create folder" className="w-full max-w-md rounded-[24px] border border-[#c4f24e]/24 bg-[#080808] p-5 shadow-[0_30px_90px_rgba(0,0,0,0.55)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[#c4f24e]">Profile folder</p>
            ...
            <input
              ...
              className="mt-2 h-12 w-full rounded-[12px] border border-white/10 bg-black/35 px-4 text-white outline-none transition focus:border-[#c4f24e]/50"
            />
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-3">
              ...
              <button type="button" className="min-h-11 rounded-[8px] bg-[#c4f24e] px-4 text-sm font-black text-black" onClick={createFolder}>Save</button>
            </div>
          </div>
        </div>
      ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/78 px-4 py-6 backdrop-blur-md sm:items-center">
          <div role="dialog" aria-modal="true" aria-label={`${activeFolder.label} folder viewer`} className="w-full max-w-4xl rounded-[28px] border border-white/10 bg-[#070707] p-5 shadow-[0_34px_100px_rgba(0,0,0,0.62)]">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[#c4f24e]">Folder</p>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/72 px-4 py-6 backdrop-blur-md sm:items-center">
          <div role="dialog" aria-modal="true" aria-label="Move to folder" className="w-full max-w-md rounded-[24px] border border-white/10 bg-[#080808] p-5">
            ...
            <div className="mt-5 grid gap-2">
              {folders.map((folder) => (
                <button key={folder.id} type="button" className="min-h-12 rounded-[10px] border border-white/10 bg-white/[0.035] px-4 text-left text-sm font-bold text-white transition hover:border-[#c4f24e]/35" onClick={() => assignMediaToFolder(folder.id)}>
                  {folder.label}
                </button>
              ))}
            </div>
