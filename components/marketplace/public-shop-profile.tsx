  ...
  return (
    <div className="space-y-4" data-testid="public-shop-profile">
      <Card className="overflow-hidden rounded-[36px] p-6 sm:p-8">
        ...
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(196, 242, 78,0.12),transparent_36%),radial-gradient(circle_at_bottom_left,rgba(255,255,255,0.06),transparent_28%)]" />
        <div className="relative grid gap-6 lg:grid-cols-[auto_1fr_auto] lg:items-center">
          {shop.profilePhotoUrl ? (
            ...
            <img
              ...
              alt={shop.name}
              className="h-28 w-28 rounded-[32px] border border-[#C4F24E]/40 object-cover shadow-[0_22px_52px_rgba(196, 242, 78,0.14)]"
            />
          ) : (
            <div className="flex h-28 w-28 items-center justify-center rounded-[32px] border border-[#C4F24E]/36 bg-[#C4F24E]/12 text-3xl font-black text-[#e4f9b8] shadow-[0_22px_52px_rgba(196, 242, 78,0.14)]">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="surface-label text-[#e4f9b8]">Verified shop</span>
            ...
            {shop.shopUsername ? <p className="mt-1 text-sm font-bold text-[#e4f9b8]">@{shop.shopUsername}</p> : null}
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link
              ...
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/10 bg-black/24 px-5 text-sm font-extrabold text-white transition hover:border-[#C4F24E]/35 hover:text-[#e4f9b8]"
            >
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_0.72fr]">
        <Card className="rounded-[36px] p-6 sm:p-8">
          ...
          {barbers.length ? (
            <div className="mt-5 rounded-[28px] border border-[#C4F24E]/16 bg-[#C4F24E]/8 p-4">
              <p className="surface-label text-[#e4f9b8]">Active team</p>
                ...
                {barbers.map((profile) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group flex min-w-[6rem] flex-col items-center gap-2 rounded-[22px] border border-white/8 bg-black/24 p-3 text-center transition hover:border-[#C4F24E]/28"
                    >
                      <Avatar
                        src={profile.profile.profilePhotoUrl}
                        alt={barberName}
                        initials={getInitials(barberName)}
                        className="h-14 w-14 rounded-full border-2 border-[#C4F24E]/55"
                      />
                      <span className="max-w-full truncate text-xs font-extrabold text-white group-hover:text-[#e4f9b8]">{barberName}</span>
                ...
                <div key={profile.barber.id} className="rounded-[28px] border border-white/8 bg-black/20 p-4">
                  <div className="flex items-start gap-4">
                    <Avatar
                      ...
                      alt={barberName}
                      initials={getInitials(barberName)}
                      className="h-16 w-16 border-2 border-[#C4F24E]/55"
                    />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="truncate text-xl font-black tracking-[-0.035em] text-white">{barberName}</p>
                        <ShieldCheck className="h-4 w-4 shrink-0 text-[#d9f985]" />
                      </div>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    <Link
                      href={`/barber/${profile.profile.username}` as Route}
                      className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/10 bg-black/24 px-4 text-sm font-extrabold text-white transition hover:border-[#C4F24E]/35 hover:text-[#e4f9b8]"
                    >
                      View Profile
                    </Link>
                    <MarketplaceTrackedActionLink
        ...
        <div className="space-y-4">
              ...
              {services.length ? services.slice(0, 6).map((item) => (
                <div key={item.service.id} className="rounded-[22px] border border-white/8 bg-black/24 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="status-pill text-[#e4f9b8]">{currency(item.service.price)}</span>
                  </div>
                </div>
              )) : (
          ...
          <Card className="rounded-[36px] p-6 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <CalendarDays className="h-5 w-5 text-[#d9f985]" />
            </div>
            {gallery.length ? (
          ...
          <Card className="rounded-[36px] p-6 sm:p-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <Star className="h-5 w-5 text-[#d9f985]" />
            </div>
            <p className="mt-4 rounded-[22px] border border-dashed border-white/10 bg-black/20 p-4 text-sm leading-6 text-white/58">
