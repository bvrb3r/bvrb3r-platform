...
function getBarberStatus(barber: ShopDashboardBarberSummary) {
  ...
  if (barber.activeAppointmentCount > 0) {
    return {
      label: "With guest",
      detail: `${barber.activeAppointmentCount} checked-in appointment${barber.activeAppointmentCount === 1 ? "" : "s"}`
    };
  }
...
export function OwnerTeamWorkspace() {
  ...
  const errorMessage = shopQuery.error
  ...
  const retentionRows = useMemo(() => intelligenceQuery.data?.barberRetention ?? [], [intelligenceQuery.data?.barberRetention]);
  const topBarbers = useMemo(() => intelligenceQuery.data?.topBarbers ?? [], [intelligenceQuery.data?.topBarbers]);
  ...
  const team = useMemo(() => {
    return barbers
      .map((barber): TeamBarberView => {
        const retention = retentionRows.find((entry) => entry.barberId === barber.id);
        const performance = topBarbers.find((entry) => entry.barberId === barber.id);
        ...
        return {
          ...
          todayPostedAmount: performance?.revenue ? Math.max(todayPostedAmount, performance.revenue) : todayPostedAmount,
          repeatClients: retention?.repeatClients ?? 0,
          atRiskClients: retention?.atRiskClients ?? 0,
          rebookingOpportunities: retention?.rebookingOpportunities ?? 0,
          utilization: barber.utilization,
          nextAppointmentStart: barber.nextAppointmentStart,
          payoutStatus: account?.operationalStatus.replaceAll("_", " ") ?? "not ready",
          payoutBlockReason: membership?.payoutBlockReason ?? null,
  ...
  }, [appointments, barberAccounts, barbers, memberships, retentionRows, topBarbers]);

  const selectedBarber = team.find((barber) => barber.id === selectedBarberId) ?? team[0] ?? null;
  const repeatClientPressure = team.reduce((sum, barber) => sum + barber.rebookingOpportunities, 0);
        ...
        <Card className="rounded-[32px] p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h3 className="mt-3 text-3xl font-semibold sm:text-5xl" data-display="true">Know who is driving revenue and who needs support.</h3>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-white/62">
                Team stays operational here: roster visibility, repeat-client signal, payout health, and clean links to the next owner action.
              </p>
            </div>
            <div className="rounded-[24px] border border-white/8 bg-black/20 px-4 py-4 text-sm text-white/66">
              ...
              <p className="mt-4 text-[11px] uppercase tracking-[0.22em] text-white/42">
                {repeatClientPressure} rebooking opportunities open
              </p>
            </div>
          </div>
            ...
            {isInitialLoading ? (
              ...
              <>
                ...
                <div className="rounded-[24px] border border-white/8 bg-black/20 p-4">
                  ...
                  <p className="mt-3 text-3xl font-semibold" data-display="true">{team.filter((barber) => barber.statusLabel === "Cutting now").length}</p>
                  <p className="mt-2 text-sm text-white/58">Barbers currently in live service.</p>
                </div>
                <div className="rounded-[24px] border border-white/8 bg-black/20 p-4">
                  <p className="surface-label">Repeat pressure</p>
                  <p className="mt-3 text-3xl font-semibold" data-display="true">{repeatClientPressure}</p>
                  <p className="mt-2 text-sm text-white/58">Clients due back without a future booking.</p>
                </div>
                <div className="rounded-[24px] border border-white/8 bg-black/20 p-4">
                  <p className="surface-label">Payout blockers</p>
                  <p className="mt-3 text-3xl font-semibold" data-display="true">{team.filter((barber) => Boolean(barber.payoutBlockReason)).length}</p>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-[22px] border border-white/8 bg-black/20 p-4">
                  ...
                  <p className="mt-3 text-2xl font-semibold">{selectedBarber.repeatClients}</p>
                  <p className="mt-2 text-sm text-white/58">{selectedBarber.rebookingOpportunities} due back • {selectedBarber.atRiskClients} at risk</p>
                </div>
                <div className="rounded-[22px] border border-white/8 bg-black/20 p-4">
                ...
                <div className="rounded-[22px] border border-white/8 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-white/58">{selectedBarber.payoutBlockReason ?? "No payout blocker recorded."}</p>
                </div>
              </div>
      ...
      <Card className="rounded-[32px] p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-white/58">Tap a barber to inspect performance, retention pressure, and payout posture without leaving the team lane.</p>
          </div>
            ...
            team.map((barber) => (
              ...
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-5">
                  ...
                  <div className="rounded-[20px] border border-white/8 bg-black/18 p-3">
                    <p className="surface-label">Repeat signal</p>
                    <p className="mt-2 text-base font-semibold">{barber.repeatClients}</p>
                  </div>
                  <div className="rounded-[20px] border border-white/8 bg-black/18 p-3">
