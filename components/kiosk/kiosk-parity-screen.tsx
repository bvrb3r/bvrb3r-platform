...
const COPY = {
  en: {
    ...
    nextHelp: "Join the live booking flow and get the earliest eligible chair.",
    ...
    cardHelp: "Your Barber will charge you from Checkout using the available card method.",
  ...
  es: {
    live: "En vivo",
    exit: "Salir",
    next: "Toma la próxima silla",
    future: "Elige una hora futura",
    choose: "Elige tu barbero",
    nextHelp: "Entra al flujo en vivo y recibe la primera silla disponible.",
    ...
    cardHelp: "Tu barbero te cobrará desde Checkout con el método disponible.",
  ...
  ht: {
    ...
    nextHelp: "Antre nan liy lan epi pran premye chèz ki disponib la.",
    ...
    cardHelp: "Babè a ap chaje w nan Checkout ak metòd kat ki disponib la.",
...
export function KioskParityScreen({ shopId, scope = "shop" }: { shopId: string; scope?: "shop" | "barber" }) {
  ...
  const eligibleBarbers = useMemo(() => (payload?.barbers ?? []).filter((barber) => barber.acceptsWalkIns && !isUnavailable(barber.waitDisplayLabel)), [payload?.barbers]);
  const publicBarbers = useMemo(() => (payload?.barbers ?? []).filter((barber) => !isUnavailable(barber.waitDisplayLabel)), [payload?.barbers]);
  const selectedBarber = useMemo(() => publicBarbers.find((barber) => barber.id === form.preferredBarberId) ?? eligibleBarbers[0] ?? publicBarbers[0], [eligibleBarbers, form.preferredBarberId, publicBarbers]);
  const selectedService = payload?.services.find((service) => service.id === form.serviceId);
  const requiredDetailsReady = Boolean(form.selectedProfileId || (form.fullName.trim() && isPhone(form.phone) && isEmail(form.email) && form.policyAccepted));
  const canSubmit = requiredDetailsReady && Boolean(form.serviceId) && (flow === "next" || Boolean(form.scheduledAt));
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden bg-[#060708] text-[#f5f1e8]" style={{ zoom: largeText ? 1.14 : 1 }}>
      ...
      <header className="relative z-20 flex flex-wrap items-center justify-between gap-3 border-b border-white/8 px-5 py-4 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-full border border-white/10 bg-black/30 p-1" aria-label="Language">
            {(["en", "es", "ht"] as const).map((item) => <button key={item} type="button" onClick={() => setLocale(item)} className={`rounded-full px-3 py-2 font-mono text-[10px] font-bold uppercase ${locale === item ? "bg-[#c4f24e] text-[#050505]" : "text-white/48"}`}>{item === "ht" ? "KRE" : item.toUpperCase()}</button>)}
          </div>
          <button type="button" onClick={() => setLargeText((value) => !value)} className={`grid h-11 w-11 place-items-center rounded-full border text-sm font-bold ${largeText ? "border-[#c4f24e]/45 bg-[#c4f24e]/12 text-[#c4f24e]" : "border-white/10 text-white/56"}`}>Aa</button>
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
        {step !== "welcome" && step !== "confirmation" ? (
          <button type="button" onClick={() => setStep(step === "details" ? "welcome" : step === "barber" ? "welcome" : step === "service" ? "details" : step === "schedule" ? "service" : "service")} className="mb-7 inline-flex items-center gap-2 text-sm font-bold text-white/52"><ArrowLeft className="h-4 w-4" />{t.back}</button>
        ) : null}

        {error ? <div className="mb-6"><FeedbackBanner tone="error" message={error} /></div> : null}

        {step === "welcome" ? (
          <div className="animate-[rise_.6s_ease-out]">
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2">
              <ChoiceCard title={t.next} body={t.nextHelp} icon={<Scissors className="h-6 w-6" />} disabled={!eligibleBarbers.length || !payload.services.length} onClick={() => { setFlow("next"); setForm((current) => ({ ...current, preferredBarberId: scope === "barber" ? publicBarbers[0]?.id ?? "" : "" })); setStep(scope === "shop" && payload.defaults.allowChooseBarber !== false ? "barber" : "details"); }} active />
              <ChoiceCard title={t.future} body={t.futureHelp} icon={<CalendarDays className="h-6 w-6" />} disabled={!publicBarbers.length || !payload.services.length} onClick={() => { setFlow("schedule"); setStep(scope === "shop" ? "barber" : "details"); }} />
            </div>
        ...
        {step === "barber" ? (
          <div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {publicBarbers.map((barber) => <button key={barber.id} type="button" onClick={() => { setForm((current) => ({ ...current, preferredBarberId: barber.id })); setStep("details"); }} className="rounded-[26px] border border-white/10 bg-white/[0.025] p-5 text-left transition hover:border-[#c4f24e]/45">
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.82fr]">
              <div className="space-y-4 rounded-[28px] border border-white/10 bg-white/[0.025] p-6">
                ...
                {form.selectedProfileId ? <div className="rounded-2xl border border-[#c4f24e]/25 bg-[#c4f24e]/8 p-4 text-sm text-[#e8f8bc]">Welcome back. Your saved phone and email will be used privately for updates.</div> : <>
                  <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.18em] text-white/48">Your name</span><Input value={form.fullName} onChange={(event) => setForm((current) => ({ ...current, fullName: event.target.value }))} placeholder="Jordan Ellis" /></label>
                  <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.18em] text-white/48">Phone · for your “you’re up” text</span><Input value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} placeholder="(813) 555-0101" inputMode="tel" /></label>
                  <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.18em] text-white/48">Email · for your receipt</span><Input value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} placeholder="name@example.com" inputMode="email" /></label>
