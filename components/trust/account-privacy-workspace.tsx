...
const COPY: Record<PrivacyState, {
  ...
  exportsent: {
    ...
    headline: "On its way",
    subline: "We’ll email the download link within 24 hours. It works for 7 days, for you only."
  },
  deactivate: {
    icon: "~",
    tone: "gold",
  ...
  confirm: {
    ...
    subline: "Type-to-confirm plus a verification code. A 30-day grace window follows — sign in during it to fully restore."
  },
  graceperiod: {
    icon: "~",
    tone: "gold",
    eyebrow: "Deletion scheduled",
    headline: "30 days to change your mind",
        ...
        return (
          <Pr27PrimaryButton
            ...
            onClick={() => run(async () => {
              ...
              setMessage("Verification code issued for 10 minutes.");
            })}
          >
            {busy ? "Checking bookings…" : "Send verification code →"}
          </Pr27PrimaryButton>
        );
      }
      return (
        ...
        >
          ...
          <label className="grid gap-2 text-xs text-white/60">
            Verification code <span className="font-mono text-[#C9A87C]">{challenge}</span>
            <input
    ...
    >
      <Pr27EvidenceCard>
        ...
        <div className="my-6 flex flex-wrap justify-center gap-2">
          ...
          {state === "confirm" ? <Fact>Grace: 30 days · then permanent</Fact> : null}
          {state === "graceperiod" ? (
            <>
        ...
        {state === "exportsent" ? (
          <p className="mt-5 text-center text-xs leading-5 text-white/42">
            The link expires after seven days and is scoped to this account only.
          </p>
        ) : null}
        {state === "deleted" ? (
          <p className="mt-5 text-center text-xs leading-5 text-white/42">
