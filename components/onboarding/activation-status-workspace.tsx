  ...
  return (
    <section className="page-shell safe-top-pad app-safe-bottom flex min-h-[100svh] min-h-[100dvh] items-start py-6 sm:items-center sm:py-10">
      <Card className="w-full rounded-[34px] p-6 sm:p-8 lg:p-10">
        ...
        <h1 className="mt-5 text-balance text-4xl font-semibold sm:text-5xl" data-display="true">Your lane is connected. Now it needs the final trust signal.</h1>
        <p className="mt-5 max-w-3xl text-sm leading-7 text-white/66 sm:text-base">
          Onboarding is complete. BVRB3R is now reading canonical verification and provider truth to decide when the lane can go publicly live.
        </p>
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          {(query.data?.lanes ?? []).map((lane) => (
            <Card key={lane.role} className="rounded-[28px] border border-white/8 bg-black/20 p-5">
              ...
              <h2 className="mt-3 text-2xl font-semibold text-white">{lane.activationState === "active" ? "Live" : "Waiting on trust checks"}</h2>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <Link href={lane.resumePath}><Button variant="secondary" className="h-12 px-6">Open lane</Button></Link>
                {lane.isActive ? <Link href={lane.dashboardPath}><Button className="h-12 px-6">Open dashboard</Button></Link> : null}
              </div>
            </Card>
          ))}
        </div>
