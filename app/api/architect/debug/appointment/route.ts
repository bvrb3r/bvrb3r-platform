...
export async function GET(request: Request) {
  ...
  try {
    const packet = await buildAppointmentDebugPacket(supabase, appointmentId, access.actor);
    return NextResponse.json(packet);
  } catch (error) {
    return NextResponse.json({
      ok: false,
      error: error instanceof Error ? error.message : "Unable to build appointment debug packet.",
      safeMessage: "Appointment debug could not be completed.",
      stage: "appointment_debug"
    }, { status: 500 });
  }
}
