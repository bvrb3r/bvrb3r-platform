...
import {
  ...
  normalizeArchitectAccountDirectoryFilters
...
function createEmptyDirectoryPayload(
...
export default async function ArchitectUsersPage({
  searchParams
}: {
  ...
  try {
    ...
    console.error("[Architect Users] directory page loader failed", error);
    initialData = createEmptyDirectoryPayload(
      initialFilters,
      ["Architect user directory failed to read live account truth. This is not a true zero-user state."]
    );
  }
