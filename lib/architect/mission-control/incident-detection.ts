  ...
  return buildRoleTruthInventory({
    ...
    rows: [
      ...
      })),
      {
        ...
        evidenceSource: `invalidProfileRoleCounts=${formatCounts(invalidRoleCounts)}; nullOrMissingProfileRoleCount=${nullOrMissingRoleCount}; roleNormalizationApprovalEvidenceStatus=Needs Review; roleNormalizationEligible=${roleNormalizationSummary.eligibleCount}; roleNormalizationBlocked=${roleNormalizationSummary.blockedCount}; manualReviewCount=${roleNormalizationApprovalPacket.manualReviewCount}; noOpCount=${roleNormalizationApprovalPacket.noOpCount}; affectedCount=${roleNormalizationApprovalPacket.affectedCount}; proposedRoleCounts=${formatCounts(new Map(Object.entries(roleNormalizationApprovalPacket.proposedRoleCounts)))}; canonicalOutputOnly=${roleNormalizationApprovalPacket.canonicalOutputOnly ? "yes" : "no"}; rawMutationExecuted=${roleNormalizationApprovalPacket.rawMutationExecuted ? "yes" : "no"}; approvalRequired=${roleNormalizationApprovalPacket.approvalRequired ? "yes" : "no"}; rollbackPlanPresent=${roleNormalizationSummary.rollbackPlanPresent ? "yes" : "no"}; rollbackPacketPresent=${roleNormalizationApprovalPacket.rollbackPacketPresent ? "yes" : "no"}; ambiguousRoles=${roleNormalizationSummary.ambiguousRoles.join(", ") || "none"}; publicOutputRedacted=${roleNormalizationApprovalPacket.publicOutputRedacted ? "yes" : "no"}; content_exposed=false.`
      },
      {
        ...baseRow,
