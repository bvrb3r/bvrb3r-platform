...
function classifyRegressionEvidence(input: {
  ...
  if (!input.expectedMainCommit || !input.runtimeCommit) return "Needs Review";
  if (!commitsMatch(input.validationCommit, input.runtimeCommit) || !commitsMatch(input.validationCommit, input.expectedMainCommit)) {
    return "Failed";
  }
  return "Pass";
}
...
function failedDeploymentEvidenceRows(input: {
  ...
  if (input.validationCommit && input.expectedMainCommit && !commitsMatch(input.validationCommit, input.expectedMainCommit)) {
...
function buildCeoCards(
  ...
  return [
    ...
    evidenceCard("critical-incidents", "Critical incidents", "CEO", "Incident Review", criticalIncidents.length ? "Failed" : checkedAt ? "Pass" : "Needs Review", criticalIncidents.length ? `${criticalIncidents.length} critical incident(s) need executive attention.` : checkedAt ? "Incident scan evidence is connected and reports zero critical incidents." : "No critical incident scan proof is connected.", criticalIncidents.map((incident) => incident.headline).concat(criticalIncidents.length ? [] : [checkedAt ? `Incident detector checkedAt=${checkedAt}; zero critical incidents found.` : "Critical incident scan proof is not connected."])),
