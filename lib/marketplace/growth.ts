...
function getServiceIdsByBarber(runtime: MarketplaceRuntimeData) { return runtime.state.services.reduce((map, service) => { const barberId = service.barberId; if (!barberId) return map; map.set(barberId, [...(map.get(barberId) ?? []), service.id]); return map; }, new Map<string, string[]>()); }
function getProofMap(runtime: MarketplaceRuntimeData, engagementState: EngagementState, filters: DiscoveryFilters = {}, trustState?: TrustState) { void engagementState; const results = searchMarketplace(runtime.state, filters, trustState); return { results, proofSignals: buildBarberProofSignals({ discoveryResults: results, rankingInputs: runtime.rankingInputs, servicePopularity: runtime.servicePopularity, conversionEvents: runtime.conversionEvents, serviceIdsByBarber: getServiceIdsByBarber(runtime), trustState }) }; }
...
export function buildMarketplaceLeaderboards(runtime: MarketplaceRuntimeData, engagementState: EngagementState, trustState?: TrustState): MarketplaceLeaderboardsPayload {
  ...
  const styleLeaders = runtime.state.styleTags.map((styleTag) => { const styleResults = rankedResults.filter((result) => runtime.state.services.some((service) => service.barberId === result.barberId && (service.styleTagIds ?? []).includes(styleTag.id))); const bookingCount = runtime.servicePopularity.filter((row) => runtime.state.services.some((service) => service.id === row.serviceId && (service.styleTagIds ?? []).includes(styleTag.id))).reduce((sum, row) => sum + row.metrics.bookingCount, 0); const trendLabel = runtime.state.trendingStyles.find((style) => style.styleTagId === styleTag.id)?.regionLabel ?? "No region signal yet"; return { styleTag, bookingCount, regionLabel: trendLabel, trendLabel: bookingCount ? `${bookingCount} marketplace bookings` : "Style signal building", barbers: styleResults.slice(0, 3).map((result) => toLeaderboardEntry(result, `${result.mostBookedService ?? styleTag.name} | ${result.reviewIntegrityLabel ?? "Integrity monitored"}`, result.rankingLabel ?? styleTag.name, getRankingLabel(engagementState, result.barberId, "style_leader") ?? result.trustLabel ?? `${styleTag.name} leader`)) }; }).filter((entry) => entry.barbers.length).sort((left, right) => right.bookingCount - left.bookingCount).slice(0, 4);
