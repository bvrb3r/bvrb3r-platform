...
function getProofMap(runtime: MarketplaceRuntimeData, engagementState: EngagementState, filters: DiscoveryFilters = {}, trustState?: TrustState) { const results = searchMarketplace(runtime.state, filters, trustState); return { results, proofSignals: buildBarberProofSignals({ discoveryResults: results, engagementState, rankingInputs: runtime.rankingInputs, servicePopularity: runtime.servicePopularity, conversionEvents: runtime.conversionEvents, serviceIdsByBarber: getServiceIdsByBarber(runtime), trustState }) }; }
