...
import { normalizeBarberSubtype, normalizeCompensationModel } from "@/lib/auth/roles";
...
async function readSupabaseRuntime(supabase: SupabaseClient): Promise<MarketplaceRuntimeData> {
  const [
    ...
    supabase.from("barbers").select("id, reference_code, profile_id, compensation_model, barber_subtype, autobooth_percent, booth_rent_amount, booth_rent_frequency, bio, booking_slug, app_approval_status, shop_approval_status"),
  ...
  const barberState: Barber[] = tableRows(barbers.data).map((row: any): Barber | null => {
    ...
    const compensationModel: Barber["compensationModel"] = normalizeCompensationModel(row.compensation_model);
    ...
    return {
      ...
      role,
      barberSubtype: normalizeBarberSubtype(row.barber_subtype),
      ...
      reviewCount: 0,
      compensationModel,
      autoBoothPercent: row.autobooth_percent ? Number(row.autobooth_percent) : undefined,
...
function createSupabaseProvider(supabase: SupabaseClient): MarketplaceProvider {
  return {
    ...
    async deleteService(actor, serviceId) { const runtime = await readSupabaseRuntime(supabase); const result = deleteServiceDefinition(runtime.state, actor, serviceId); await syncServiceToCanonicalRows(supabase, result.service); await refreshDerivedSignals(supabase); return { service: result.service }; },
