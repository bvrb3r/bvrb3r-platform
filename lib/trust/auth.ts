...
export async function requireTrustActor(allowedRoles: Role[]) {
  ...
  return { role: normalizeAccountRole(user.role), barberId: user.barberId, clientId: user.clientId, locationIds: user.locationIds, userEmail: user.email } satisfies TrustActor;
}
